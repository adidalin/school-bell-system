...
def init_system():
    ...
    # 配置 VoiceMeeter 路由：Virtual Input 1 (VAIO) → A1(Speakers), Virtual Input 2 (AUX) → A2(Headphones)
    _configure_voicemeeter()
...
def _configure_voicemeeter():
    """配置 VoiceMeeter Banana 路由：VAIO→A1(Speakers), AUX→A2(Headphones)"""
    try:
        import ctypes, sys
        bits = 64 if sys.maxsize > 2**32 else 32
        dll = f"C:/Program Files (x86)/VB/Voicemeeter/VoicemeeterRemote{bits}.dll"
        logger.info("配置 VoiceMeeter VBAN...")
        vm = ctypes.windll.LoadLibrary(dll)
        if vm.VBVMR_Login() != 0:
            logger.warning("VoiceMeeter 未运行，跳过配置")
            return
        vm.VBVMR_SetParameters(b"Strip[3].A1=1;Strip[3].A2=0;Strip[3].A3=0;Strip[3].B1=0;Strip[3].B2=0\0")
        vm.VBVMR_SetParameters(b"Strip[4].A1=0;Strip[4].A2=1;Strip[4].A3=0;Strip[4].B1=0;Strip[4].B2=0\0")
        vm.VBVMR_SetParameters(b"Bus[0].device.wdm=1\0")
        vm.VBVMR_SetParameters(b"Bus[1].device.ks=1\0")
        vm.VBVMR_Logout()
        logger.info("VoiceMeeter 配置完成: VAIO→A1(Speakers), AUX→A2(Headphones)")
    except Exception as e:
        logger.warning(f"VoiceMeeter 配置失败: {e}")


def _ensure_voicemeeter_running():
    """如果 VoiceMeeter 未运行则自动启动"""
    import ctypes, sys
    ...
    try:
        ...
        pass
    vm_exe = "C:/Program Files (x86)/VB/Voicemeeter/voicemeeterpro.exe"
    if os.path.exists(vm_exe):
        subprocess.Popen([vm_exe], shell=False)
        logger.info("VoiceMeeter 已启动")
    else:
        logger.warning(f"VoiceMeeter 未安装: {vm_exe}")


def _load_zone_devices():
    """从数据库加载区域音频设备分配到内存"""
    from models import get_db
    conn = get_db()
    zones = conn.execute("SELECT id, audio_device FROM zones").fetchall()
    for z in zones:
        engine.player.zone_devices[z["id"]] = z["audio_device"]
        logger.info(f"区域 {z['id']} 设备: {z['audio_device']}")
    conn.close()
