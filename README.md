...
├── vm_setup.py            # VoiceMeeter 路由配置工具（手动运行）
├── requirements.txt       # Python 依赖
├── install_autostart.ps1  # 开机自启安装脚本
├── start_bell.vbs         # VBS 隐藏窗口启动器
├── start_bell.bat         # 批处理启动脚本
├── templates/
│   ├── index.html         # 前端主界面（单页应用）
│   └── login.html         # 登录界面
├── static/sounds/         # 铃声音频文件（.wav）
...
### 方案一（推荐）：板载双孔声卡 + VoiceMeeter（0 元软件成本）

大多数台式机 Realtek ALC 系列声卡支持"前后独立输出"：

1. 安装 **Realtek HD Audio 驱动**，在 Realtek HD Audio Manager 中开启*"使前部和后部输出设备同时播放不同的音频流"*
2. 安装 **VoiceMeeter Banana**（免费虚拟音频混音器），用于将一路虚拟输入路由到前部（绿色孔），另一路路由到后部（黑色孔）
3. 系统自动识别为两个独立 WDM-KS 输出设备

```
Python 音频引擎
  ├── Zone A ── WASAPI ── Voicemeeter Input ── A1 Bus ── 绿色孔(前置) ── 功放1 ── 教学楼喇叭
  └── Zone B ── WASAPI ── Voicemeeter AUX Input ── A2 Bus ── 黑色孔(后置) ── 功放2 ── 操场喇叭
```

> **为什么需要 VoiceMeeter？** WDM-KS（内核流）独占模式下，Windows 不允许同一进程同时打开两个不同物理输出。VoiceMeeter 创建了两个 WASAPI 虚拟输入，支持共享模式多路并发，并在内部将两路音频路由到不同的物理 KS 引脚。
...
### 环境要求
- **Windows 10/11**（必选，依赖 VoiceMeeter 和 WASAPI）
- **Python 3.9+**
- **VoiceMeeter Banana**（免费，[官网下载](https://vb-audio.com/Voicemeeter/banana.htm)）
- 2GB+ 内存 — 100MB 磁盘

### 安装步骤

1. **安装 VoiceMeeter Banana**（管理员身份运行安装包，重启电脑）
2. **安装 Python 依赖**：
   ```bash
...
> 首次启动会自动检测并启动 VoiceMeeter，配置虚拟输入路由（Strip[3]→A1 音箱, Strip[4]→A2 耳机）。
...
```
> 如需 NTP 精准校时，首次运行 `python run.py` 时以管理员身份执行（仅首次校时需要管理员权限）。
...
| 音频 | sounddevice (PortAudio) + numpy | WASAPI 共享模式多路混音输出 |
| 虚拟音频 | VoiceMeeter Banana | 将一路虚拟输入分别路由到不同物理输出 |
...
**Q: 双区域同时不同铃声怎么实现？**
A: 需声卡支持前端/后端独立输出（Realtek 驱动中开启），配合 **VoiceMeeter Banana**（免费）进行虚拟路由。系统启动时自动配置：Zone A → Voicemeeter Input → 前置孔 → 音箱，Zone B → Voicemeeter AUX Input → 后置孔 → 耳机。如果使用 USB 声卡扩展（最多 8 路），每个 USB 声卡也是一个独立设备。
...
**Q: 断电重启后会自动恢复吗？**
A: 运行 `install_autostart.ps1` 安装开机自启后，每次 Windows 登录会自动启动 python 程序和 VoiceMeeter，无需人工干预。
