...
# ====== 音频播放模块（支持多物理声卡独立输出） ======
class AudioPlayer:
    """多区域音频播放器 - 每物理设备独立 OutputStream 混音"""
    ...
    def _resample(self, data, orig_sr):
        if orig_sr == self.TARGET_SR:
            return data
        n = int(len(data) * self.TARGET_SR / orig_sr)
        x_old = np.arange(len(data))
        x_new = np.linspace(0, len(data) - 1, n)
        if data.ndim == 1:
            return np.interp(x_new, x_old, data).astype(np.float32)
        result = np.zeros((n, data.shape[1]), dtype=np.float32)
        for c in range(data.shape[1]):
            result[:, c] = np.interp(x_new, x_old, data[:, c])
        return result

    def _stream_key(self, device_id):
        """设备ID → 流标识"""
    ...
    def _get_or_create_stream(self, device_id):
        ...
        def make_callback(skey):
            def cb(outdata, frames, time_info, status):
                outdata.fill(0)
                if status:
                    logger.warning(f"Audio [{skey}]: {status}")
                done = []
                with self._lock:
                    for zid, job in list(self._jobs.items()):
                        if job.get('stream') != skey:
                            continue
                        n = min(frames, len(job['data']) - job['pos'])
                        if n > 0:
                            chunk = job['data'][job['pos']:job['pos'] + n]
                            if chunk.ndim == 1:
                                for ch in range(outdata.shape[1]):
                                    outdata[:n, ch] += chunk[:n]
                            else:
                                for ch in range(min(outdata.shape[1], chunk.shape[1])):
                                    outdata[:n, ch] += chunk[:n, ch]
                            job['pos'] += n
                        if job['pos'] >= len(job['data']):
                            done.append(zid)
                    for zid in done:
                        del self._jobs[zid]
                for zid in done:
                    self.zone_playing[zid] = False
        ...
        try:
            ...
            logger.info(f"音频流已创建 [{key}]")
            return st
        except Exception as e:
            logger.error(f"创建音频流失败 [{key}]: {e}")
    ...
    def _device_id(self, zone_id):
        """根据 zone_devices 存储的设备标识, 查找当前 WASAPI 设备索引"""
        ident = self.zone_devices.get(zone_id, "")
        ...
        if ident.startswith("name:"):
            ...
            try:
                ...
                if 0 <= idx < len(devs) and devs[idx]['max_output_channels'] > 0:
                    return idx
            except ValueError:
                pass
            target_name = ident
        if target_name:
            ...
            # Search WASAPI first (VoiceMeeter virtual devices)
            for ha in hostapis:
                if 'WASAPI' in ha['name']:
                    for idx in ha['devices']:
                        d = devs[idx]
                        if d['max_output_channels'] > 0 and d['name'].strip() == target_name:
                            return idx
            # Then WDM-KS (physical outputs)
            for ha in hostapis:
    ...
    def _read_audio(self, filepath):
        ...
        if ext in ('.wav', '.flac', '.ogg'):
            ...
            return self._resample(data, sr), self.TARGET_SR
        elif ext == '.mp3':
            try:
                import subprocess, numpy as np, io
                p = subprocess.run(['ffmpeg', '-i', filepath, '-f', 'wav', '-acodec', 'pcm_f32le', '-'],
                ...
                try:
                    ...
                    if not pygame.mixer.get_init():
                        pygame.mixer.init(frequency=self.TARGET_SR, size=-16, channels=2, buffer=2048)
                    ...
                    return self._resample(arr, sr), self.TARGET_SR
                except Exception as e2:
                    raise ValueError(f"MP3播放失败(需安装ffmpeg或使用WAV格式): {e2}")
    ...
    def play(self, zone_id, filepath, volume=1.0, blocking=False):
        ...
        if not ok:
            logger.error(f"音频检查失败: {filepath} - {msg}")
            return False
        try:
            data, sr = self._read_audio(filepath)
        except Exception as e:
            logger.error(f"读取音频失败 {filepath}: {e}")
            return False

        vol = min(max(volume, 0.0), 1.0) * self.zone_volumes.get(zone_id, 1.0)
        vol = min(max(vol, 0.0), 1.0)

        if data.ndim == 1:
            data_play = data.astype(np.float32)
        else:
            data_play = data.astype(np.float32)

        dev_id = self._device_id(zone_id)
        stream = self._get_or_create_stream(dev_id)
        if stream is None:
            logger.error(f"区域{zone_id}播放失败: 无法创建音频流")
            return False

        skey = self._stream_key(dev_id)
        with self._lock:
            self._jobs[zone_id] = {
                'data': data_play,
                'pos': 0,
        ...
        dev_name = self.zone_devices.get(zone_id, "default")
        logger.info(f"区域{zone_id}播放: {os.path.basename(filepath)} 音量{vol:.0%} 设备:{dev_name}")

        if blocking:
            import time
            while self.zone_playing.get(zone_id, False):
                time.sleep(0.1)
        return True
    ...
    def stop_all(self):
        with self._lock:
            self._jobs.clear()
        for zid in list(self.zone_playing.keys()):
            self.zone_playing[zid] = False
